module Hue
  class Group
    ...
    # The Mired Color temperature of the group. 2012 connected groups
    ...
    # * `none` – The group is not performing an alert effect.
    # * `select` – The group is performing one breathe cycle.
    # * `lselect` – The group is performing breathe cycles for 30 seconds
    #     or until an "alert": "none" command is received.
    #
    # Note that in version 1.0 this contains the last alert sent to the
    # group and not its current action. This will be changed to contain the
    # current action in an upcoming patch.
    #
    # @see http://developers.meethue.com/coreconcepts.html#some_extra_fun_stuff
    attr_accessor :alert

    # The dynamic effect of the group, can either be `none` or
    # `colorloop`. If set to colorloop, the group will cycle through
    ...
    # @param transition The duration of the transition from the group’s current
    ...
    # Refresh the state of the group
